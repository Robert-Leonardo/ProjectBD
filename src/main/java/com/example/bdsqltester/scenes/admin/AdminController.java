...
import javafx.scene.Parent; // Tambahkan import Parent
...
public class AdminController {
    ...
    void InputDataSiswaButton(ActionEvent event) {
        try {
            ...
            Parent root = loader.load(); // Gunakan Parent
            // Teruskan currentUser ke AdminAccController
            AdminAccController controller = loader.getController();
            controller.setUser(user); // Meneruskan objek user yang login
    ...
    void inputJadwalButton(ActionEvent event) {
        try {
            ...
            Parent root = loader.load(); // Gunakan Parent
            // Teruskan currentUser ke AdminJadwalController
    ...
    // Metode baru untuk kelola kelas
    @FXML
    void onKelolaKelasClick(ActionEvent event) {
        try {
            HelloApplication app = HelloApplication.getApplicationInstance();
            app.getPrimaryStage().setTitle("Admin - Kelola Kelas & Siswa");

            FXMLLoader loader = new FXMLLoader(HelloApplication.class.getResource("adminKelolaKelas-view.fxml")); // Muat FXML baru
            Parent root = loader.load();
            AdminKelolaKelasController controller = loader.getController();
            controller.setUser(user);
            Scene scene = new Scene(root);
            app.getPrimaryStage().setScene(scene);
        } catch (IOException e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Error Memuat Tampilan", "Tidak dapat membuka halaman pengelolaan kelas.");
        }
    }
